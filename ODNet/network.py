...
class ODNet(nn.Module):
    ...
    def forward(self, im1, im2, coord1):
        ...
        xc1, xf1, f11, f12 = self.net(im1)
        xc2, xf2, f21, f22 = self.net(im2)
    ...
    def extract_features(self, im, coord):
        ...
        '''
        xc, xf, f1, f2 = self.net(im)
        hi, wi = im.size()[2:]
        coord_n = self.normalize(coord, hi, wi)
        feat_c = self.sample_feat_by_coord(xc, coord_n)
        feat_f = self.sample_feat_by_coord(xf, coord_n)
        feat_f1 = self.sample_feat_by_coord(f1, coord_n)
        feat_f2 = self.sample_feat_by_coord(f2, coord_n)

        Lx = torch.sqrt(torch.sum(torch.mul(feat_f1, feat_f1).squeeze(0), 1))
        Ly = torch.sqrt(torch.sum(torch.mul(feat_f2, feat_f2).squeeze(0), 1))
        cos_angle = torch.sum(torch.mul(feat_f1, feat_f2).squeeze(0), 1) / (Lx * Ly)
        angle = torch.acos(cos_angle) * 360 / 2 / np.pi
    ...
    def test(self, im1, im2, coord1):
        ...
        xc1, xf1, _, _ = self.net(im1)
        xc2, xf2, _, _ = self.net(im2)
...
class ResUNet(nn.Module):
    def __init__(self,
        ...
        # add 2 3x3 convs and 1 1x1 conv
        self.conv_3x3_1_1 = nn.Sequential(
            nn.Conv2d(fine_out_ch, fine_out_ch, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(fine_out_ch),
            nn.ReLU()
        )
        self.conv_3x3_1_2 = nn.Sequential(
            nn.Conv2d(fine_out_ch, fine_out_ch, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(fine_out_ch),
            nn.ReLU()
        )
        self.conv_1x1_1 = nn.Sequential(
            nn.Conv2d(fine_out_ch, fine_out_ch, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(fine_out_ch),
            nn.ReLU()
        )
        self.conv_3x3_2_1 = nn.Sequential(
            nn.Conv2d(fine_out_ch, fine_out_ch, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(fine_out_ch),
            nn.ReLU()
        )
        self.conv_3x3_2_2 = nn.Sequential(
            nn.Conv2d(fine_out_ch, fine_out_ch, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(fine_out_ch),
            nn.ReLU()
        )
        ...
        # self.conv_3x3 = conv(fine_out_ch, fine_out_ch, 3, 1)
        # self.conv_1x1 = conv(fine_out_ch, fine_out_ch, 1, 1)
    ...
    def forward(self, x):
        ...
        x_fine_org = self.conv_fine(x)

        x_fine_1 = self.conv_3x3_1_1(x_fine_org)
        x_fine_1 = self.conv_3x3_1_2(x_fine_1)
        x_fine_1 = self.conv_1x1_1(x_fine_1)

        x_fine_2 = self.conv_3x3_2_1(x_fine_org)
        x_fine_2 = self.conv_3x3_2_2(x_fine_2)
        x_fine_2 = self.conv_1x1_2(x_fine_2)

        x_fine = x_fine_1 + x_fine_2
        # x_fine = torch.cat([x_fine_1, x_fine_2], dim=1)
        return [x_coarse, x_fine, x_fine_1, x_fine_2]
